...
const similarities = (a: string, b: string): string => {
    return a.split('').filter(item => b.includes(item)).join('')
}
...
export const part1 = (input: string): number => {
    ...
    const items = sacks.map(([top, bottom]) => similarities(top, bottom).slice(0,1));
    return items.map(getLetterCode).reduce((total, priority) => total + priority, 0);
}
...
export const part2 = (input: string): number => {
    ...
    for (let i=0; i<(sacks.length/3); i++) {
        ...
        badges.push(similarities(similarities(group[0], group[1]), group[2]).slice(0,1))
    }
    return badges.map(getLetterCode).reduce((total, priority) => total + priority, 0);
}